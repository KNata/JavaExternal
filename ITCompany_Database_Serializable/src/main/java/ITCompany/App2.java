...
public class App2 {
    ...
    public static void main(String[] args) throws SQLException, ClassNotFoundException, IOException {
        ...
        while (true) {
            ...
            writeListOfLaptopsToFile(theDAO.findAll());

        }


    }
    private static void writeListOfLaptopsToFile(List<Laptop> laptopList) throws IOException{
        FileOutputStream fos = null;
        ObjectOutputStream oos = null;
        try {
            fos = new FileOutputStream("laptopList.txt");
            oos = new ObjectOutputStream(fos);
            for(Laptop lapt : laptopList) {
                oos.writeObject(lapt.toString());
            }
        }catch (IOException e) {
